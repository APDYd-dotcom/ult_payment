...
function translateDepartmentError($message)
{
    ...
    // Fallback: return the raw database message safely escaped
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['Create'])) {
        ...
        if (empty($Name)) {
            ...
            $checkStmt = $bdd->prepare("SELECT COUNT(*) FROM department WHERE name = ?");
            $checkStmt->execute([$Name]);
            if ($checkStmt->fetchColumn() > 0) {
                $message = "Ce nom de département existe déjà. Veuillez en choisir un autre.";
                $messageType = "error";
            } else {
                try {
                ...
                <div class="crud-container">
                    <div class="table-section">
                        <div class="search-container">
                            <div class="search-box">
                                ...
                                <input id="payment-search" type="text" placeholder="Search departments..."
                                    aria-label="Search departments" />
                                <button type="button" id="clear-payment-search" class="clear-btn"
                                    aria-label="Clear search">
                                    <span class="clear-icon">✕</span>
                                </button>
                            </div>
                            <div class="search-results-counter" id="search-counter">
                                Found <strong id="counter-match">0</strong> of <span id="counter-total">0</span>
                                departments
                            </div>
                        </div>
                        <table>
                                    ...
                                    <?php foreach ($departments as $row): ?>
                                        <tr data-id="<?= htmlspecialchars($row['id']) ?>"
                                            ...
                                            <td class="student-name-cell">
                                                <span
                                                    class="student-name-text"><?= htmlspecialchars($row['name'] ?? '') ?></span>
                                                <div class="row-quick-actions">
                                                    <button type="button" class="quick-action-btn quick-edit-btn"
                                                        title="Modifier">✏️</button>
                                                    <button type="button" class="quick-action-btn quick-delete-btn"
                                            ...
                                            <td class="options-cell">
                                                ...
                                                <div class="options-menu dropdown-hidden">
                                                    ...
                                                    <button type="button"
                                                        class="menu-item dropdown-delete-btn">Supprimer</button>
                                                </div>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>
                    ...
                    <div class="form-section">
                        ...
                        <form method="POST" action="departement.php">
                            ...
                            <input id="minerval" type="number" step="0.01" name="minerval" required
