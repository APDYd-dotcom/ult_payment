...
try {
    $stmt = $bdd->query("SELECT * FROM vw_penalites ORDER BY penalite_created_at DESC");
    $penalties = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    die('Erreur lors de la récupération des pénalités : ' . $e->getMessage());
}
?>
<!DOCTYPE html>
<html>
                        ...
                        <tbody>
                            <?php if ($penalties): ?>
                                <?php foreach ($penalties as $row): ?>
                                    <tr>
                                        <td><?= htmlspecialchars($row['penalite_reference']) ?></td>
                                        <td><?= htmlspecialchars($row['payment_reference']) ?></td>
                                        <td><?= htmlspecialchars($row['matricule']) ?></td>
                                        <td><?= htmlspecialchars($row['student_name']) ?></td>
                                        <td><?= number_format($row['payment_amount'], 2) ?> BIF</td>
                                        <td><?= htmlspecialchars($row['retard_jours']) ?> jours</td>
                                        <td><?= number_format($row['pourcentage_penalite'], 2) ?>%</td>
                                        <td style="color: #dc2626; font-weight: 600;"><?= number_format($row['montant_penalite'], 2) ?> BIF</td>
