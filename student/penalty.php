...
try {
    $stmt = $bdd->prepare("
        SELECT vp.*
        FROM vw_penalites vp
        JOIN student s ON s.matricule = vp.matricule
        WHERE s.id = ?
        ORDER BY vp.penalite_created_at DESC
    ");
    $stmt->execute([$studentId]);
    $penalties = $stmt->fetchAll(PDO::FETCH_ASSOC);
} catch (PDOException $e) {
    die('Erreur lors de la récupération des pénalités : ' . htmlspecialchars($e->getMessage(), ENT_QUOTES, 'UTF-8'));
}
?>
<!DOCTYPE html>
<html>
<head>
    ...
    <link rel="stylesheet" href="../admin/styles.css?v=1.1">
                ...
                <div class="table-section" style="flex: 1;">
                    <div class="search-container">
                        ...
                        <div class="search-results-counter" id="search-counter">
                            Found <strong id="counter-match">0</strong> of <span id="counter-total">0</span> penalties
                        </div>
                    </div>
                    <table>
                        <thead>
                        ...
                        <tbody>
                            <?php if ($penalties): ?>
                                <?php foreach ($penalties as $row): ?>
                                    <tr>
                                        <td><?= htmlspecialchars($row['penalite_reference'], ENT_QUOTES, 'UTF-8') ?></td>
                                        ...
                                        <td><?= number_format((float) $row['payment_amount'], 2) ?> BIF</td>
                                        <td><?= htmlspecialchars($row['retard_jours'], ENT_QUOTES, 'UTF-8') ?> jours</td>
                                        <td><?= number_format((float) $row['pourcentage_penalite'], 2) ?>%</td>
                                        <td style="color: #dc2626; font-weight: 600;"><?= number_format((float) $row['montant_penalite'], 2) ?> BIF</td>
                                        <td><?= date('Y-m-d H:i', strtotime($row['penalite_created_at'])) ?></td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <tr><td colspan="9">Aucune pénalité trouvée.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
